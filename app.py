...
def precipitation():
    ...
    twelve_month_data=session.query(measurement.date, func.max(measurement.prcp)).filter(measurement.date >=query_date).group_by(measurement.date).order_by(measurement.date)
...
def stations():
    ...
    station_join = session.query(*sel).filter(station.station==measurement.station).distinct().all()
...
def start_date(yyyymmdd):
    session = Session(engine)
    start_tobs = session.query(measurement.date, func.min(measurement.tobs),func.max(measurement.tobs), func.avg(measurement.tobs)).filter(measurement.date >= yyyymmdd).group_by(measurement.date).order_by(measurement.date).all()
...
def two_dates(startdate, enddate):
    session = Session(engine)
    two_date_tobs=session.query(measurement.date, func.min(measurement.tobs),func.max(measurement.tobs), func.avg(measurement.tobs)).filter(measurement.date >= startdate).filter(measurement.date<= enddate).group_by(measurement.date).order_by(measurement.date).all()
